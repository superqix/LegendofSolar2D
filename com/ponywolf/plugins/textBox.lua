...
function M.new(instance, options) --text, wrap, align, color)
  ...
  function instance:update(newText)    
    ...
    for i = 1, #words do
      ...
      if (x + space) > wrap then
        x = 0
        y = y + lineHeight
        
        print(width, #instance.word, i)
        local w = width - (i > 1 and (instance.word[i-1].x + instance.word[i-1].width) or 0)
        ...
        lf = i
        instance.totalLines = instance.totalLines + 1
      end
    ...
    -- last line
    if not instance.word or not instance.word[#instance.word] then
      return
    end
