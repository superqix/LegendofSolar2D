...
function M.new(listener, options)
  ...
  function instance:refresh()
    ...
    for i = 1, #items do
      ...
      items[i].object:translate(0, (i > 1) and (items[i-1].object.contentHeight * 1.1) * (i-1) or 0)
  ...
  function instance:update(index)
    ...
    local selected = { time = 0, xScale = 1.2, yScale = 1.2, alpha = 1 }
    local normal = { time = 0, xScale = 1, yScale = 1, alpha = 0.5 }
  ...
  local function key(event)
    ...
    if phase == "up" then
      ...
      elseif name == "down" then
        instance:down()      
      elseif name == "enter" or name == "space" or name == "buttonA" or name =="button1" then
        local item = instance:choose() 
        if listener then
          listener( { phase = "selected", name = item } )
        else
